...
///
/// Architecture:
/// RulePacks/ (TypeScript files) → Compilation → JavaScriptCore (Execution) → SQLite (Analytics)
///
class FileBasedRuleEngine {
    ...
    init(database: RuleDatabase) {
        ...
        print("🟢 FileBasedRuleEngine initialized with file-based TypeScript rules")
    }
    ...
    private func setupJavaScriptEnvironment() {
        ...
        context.exceptionHandler = { context, exception in
            print("❌ JavaScript Exception: \(exception?.toString() ?? "Unknown error")")
        }
        ...
        print("🔧 JavaScript environment configured for file-based rules")
    }
    ...
    func loadRules() -> [FileBasedRule] {
        print("📂 Loading rules from RulePacks...")
        
        var rules: [FileBasedRule] = []
        
        // Find all rule files
        let ruleFiles = findRuleFiles()
        print("🔍 Found \(ruleFiles.count) rule files")
        ...
        print("📋 Loaded \(rules.count) rules successfully")
        return rules
    }

    /// Reload rules from files (on-demand hot reload)
    func reloadRules() -> [FileBasedRule] {
        print("🔄 Reloading rules from RulePacks...")
    ...
    private func loadRuleFromFile(_ fileURL: URL) -> FileBasedRule? {
        do {
            ...
            print("❌ Failed to load rule from \(fileURL.lastPathComponent): \(error)")
            return nil
        }
    }

    private func compileTypeScriptToJavaScript(_ sourceCode: String, fileURL: URL) -> String {
        // TypeScript to JavaScript compilation for JavaScriptCore
        // Converts ES6 modules to plain JavaScript functions
        
        var jsCode = sourceCode
        
        // 1. Remove interface definitions only - very precise multiline match
        jsCode = jsCode.replacingOccurrences(
            of: "interface\\s+[a-zA-Z_][a-zA-Z0-9_]*\\s*\\{[\\s\\S]*?\\n\\}",
            with: "",
            options: .regularExpression
        )
        
        // 2. Convert ES6 export to plain function declaration
        jsCode = jsCode.replacingOccurrences(
            of: "export\\s+function\\s+validate\\s*\\(",
            with: "function validate(",
            options: .regularExpression
        )
        
        // 3. Remove type annotations ONLY from function parameters
        jsCode = jsCode.replacingOccurrences(
            of: "(\\w+)\\s*:\\s*[a-zA-Z_][a-zA-Z0-9_<>\\[\\]|]*\\s*(?=[),])",
            with: "$1",
            options: .regularExpression
        )
        
        // 4. Remove return type annotations - more aggressive pattern
        jsCode = jsCode.replacingOccurrences(
            of: "\\)\\s*:\\s*[a-zA-Z_][a-zA-Z0-9_<>\\[\\]|]*\\s*\\{",
            with: ") {",
            options: .regularExpression
        )
        
        // 5. Remove generic type constraints but be very careful
        jsCode = jsCode.replacingOccurrences(
    ...
    func executeAllRules(fileData: FileAnalysisData) -> [ArchitecturalFinding] {
        ...
        print("🔧 Executing \(loadedRules.count) rules against \(fileData.fileName)")
        ...
        for rule in loadedRules {
            ...
            print("⏱️ Rule '\(rule.name)' executed in \(String(format: "%.3f", duration))s")
    ...
    private func loadFileDataIntoJS(_ fileData: FileAnalysisData, rule: FileBasedRule) {
        ...
        guard let jsonData = try? JSONSerialization.data(withJSONObject: fileDataDict),
            ...
            print("❌ Failed to serialize file data for rule \(rule.name)")
            return
        }
